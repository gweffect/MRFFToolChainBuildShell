...
cmake -S ${MR_BUILD_SOURCE}                         \
    ...
    -DOHOS_NDK=${MR_OHOS_NDK_HOME}            \
    -DOHOS_ABI=${MR_OHOS_ABI}                 \
    -DCMAKE_RANLIB=${MR_RANLIB}                     \
    -DCMAKE_AR=${MR_AR}                             \
    -DCMAKE_STRIP=${MR_STRIP}                       \
    -DCMAKE_CXX_COMPILER_RANLIB=${MR_RANLIB}        \
    -DOHOS_PLATFORM=ohos-${MR_OHOS_API}    \
    -DOHOS_STL=c++_shared                        \
    -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=${MR_BUILD_PREFIX}/libs/${MR_OHOS_ABI}       \
    -DCMAKE_TOOLCHAIN_FILE=${MR_OHOS_NDK_HOME}/build/cmake/ohos.toolchain.cmake \
    ${CMAKE_OTHER_OPTS}
